    ...
    <div class="col-sm-10" style="background-color:lightgrey;">
      ...
      <ul class="timeline">
        <li>
		  ...
		  <h3>Processing Posts</h3>
		  <?php
			$con = mysqli_connect("localhost","root","","1316441");
			$i = 1;
			$query = mysqli_query($con,"select * from user_donation where status = 'processing' order by 1 desc");
			...
			<tr>
				...
				{ ?>
					<a href="processing.php?status3=<?php echo $row['id'];?>"
				class="act" onclick="return confirm('Complete <?php echo $row['first_name'];?>');"> Processing </a>
				<?php
				}
				if(($status)=='completed')
				{ ?>
					<a href="processing.php?status3=<?php echo $row['id'];?>"
					class="deact" onclick="return confirm('Process <?php echo $row['first_name'];?>');"> Completed</a>
				<?php
				} ?>
				</td>
			</tr>
...
if(isset($_GET['status3']))
{
	...
	$select=mysqli_query($con,"select * from user_donation where id='$status4'");
	while($row=mysqli_fetch_array($select))
	{
		$status_var = $row['status'];
	if($status_var=='processing')
	{
		$status_state='completed';
		$update=mysqli_query($con,"update user_donation set status='$status_state' where id='$status4' ");
		exit();
	}
